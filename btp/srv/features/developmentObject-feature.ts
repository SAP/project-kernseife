import { DevelopmentObject, Import, ScoringRecord } from '#cds-models/kernseife/db';
...
export const determineCleanCoreLevel = async (developmentObject): Promise<string> => {

  //LOG.info("determineCleanCoreLevel", { developmentObject });

  const result = await SELECT.from(entities.ScoringRecords)
    .columns(`releaseState.releaseLevel_code as releaseLevel`, `count(*) as count`)
    .where({
  ...
  if ((releaseLevelMap.get("RELEASED") + releaseLevelMap.get("CLASSIC")) === totalFindings) {
    return "B";
  }

  // Level C => only Released, Classic and Tier3 objects
  if ((releaseLevelMap.get("RELEASED") + releaseLevelMap.get("CLASSIC") + releaseLevelMap.get("INTERNAL")) === totalFindings) {
    return "C";
  }

  // Level C => no "not to be used objects"
  if (releaseLevelMap.get("NOT_TO_BE_RELEASED") === 0 && releaseLevelMap.get("NO_API") === 0) {
    return "C";
  }
...
export const getDevelopmentObjectCount = async () => {
  const result = await SELECT.from(entities.DevelopmentObjects).columns("IFNULL(COUNT( * ),0) as count");
...
export const getTotalScore = async () => {
  const result = await SELECT.from(entities.DevelopmentObjects).columns("IFNULL(SUM( score ), 0) as score");
...
export const determineCleanCoreLevelByRef = async (ref) => {
  ...
  if (developmentObject.cleanCoreLevel_code !== cleanCoreLevel) {
    await await UPDATE.entity(entities.DevelopmentObjects, developmentObject.ID)
      .set({
...
export const calculateCleanCoreScore = async (developmentObject): Promise<number> => {
  // Get Latest Scoring Run
  const averageReleaseScore = await SELECT.from(entities.ScoringRecords).columns("avg(classification.releaseLevel.score)").where({
    import_ID: developmentObject.latestScoringImportId,
    objectType: developmentObject.objectType,
    objectName: developmentObject.objectName,
    devClass: developmentObject.devClass,
  ...
  if (!averageReleaseScore || averageReleaseScore.length !== 1 || !averageReleaseScore[0].avg) return 0;
...
export const calculateScoreByRef = async (ref) => {
  ...
  // Get Latest Scoring Run
  const scoringRecordList = await SELECT.from(entities.ScoringRecords).columns("itemId", "rating_code", "classification.rating.code as rating_code_dyn", "classification.rating.score as score").where({
    import_ID: developmentObject.latestScoringImportId,
    objectType: developmentObject.objectType,
    objectName: developmentObject.objectName,
    devClass: developmentObject.devClass,
  ...
  developmentObject.cleanCoreLevel_code = await determineCleanCoreLevel(developmentObject);
  developmentObject.cleanCoreScore = await calculateCleanCoreScore(developmentObject);
  LOG.info("Development Object Score", { score: developmentObject.score, cleanCoreLevel: developmentObject.cleanCoreLevel_code, cleanCoreScore: developmentObject.cleanCoreScore });
  ...
  for (const scoringRecord of scoringRecordList) {
    if (scoringRecord.rating_code_dyn !== scoringRecord.rating_code) {
      ...
      await UPDATE.entity(entities.ScoringRecords)
        ...
        .where({ import_ID: developmentObject.latestScoringImportId, itemId: scoringRecord.itemId });
    }
  }
...
export const determineNamespaceAll = async () => {
  await db.run("UPDATE kernseife_db_DEVELOPMENTOBJECTS SET NAMESPACE = CASE SUBSTRING(OBJECTNAME,1,1) WHEN 'Z' THEN 'Z' WHEN 'Y' THEN 'Y' WHEN '/' THEN SUBSTR_REGEXPR('(^/.*/).+$' IN OBJECTNAME GROUP 1) ELSE ''  END");
...
export const getDevelopmentObjectIdentifier = (object: ScoringRecord | DevelopmentObject) => {
  return (object.systemId || "") + (object.devClass || "") + (object.objectType || "") + (object.objectName || "");
};
...
export const getDevelopmentObjectMap = async () => {
  ...
  return developmentObjectDB.reduce((map, developmentObject) => {
    return map.set(getDevelopmentObjectIdentifier(developmentObject), developmentObject);
  }, new Map<string, DevelopmentObject>()) as Map<string, DevelopmentObject>;
};



export const importScoring = async (scoringImport: Import, tx?: Transaction, updateProgress?: (progress: number) => Promise<void>) => {
  ...
  const scoringRecordList = result.data
    .map((finding) => {
      if (itemIdSet.has(finding.itemId || finding.ITEMID || finding.itemID)) {
        // duplicate!
        throw new Error("Duplicate ItemId " + (finding.itemId || finding.ITEMID || finding.itemID));
      }
      ...
      return {
        ...
        refApplicationComponent: finding.refApplicationComponent || finding.REFAPPLICATIONCOMPONENT,
        rating_code: finding.rating || finding.RATING || finding.ratingCode || finding.RATINGCODE,
      } as ScoringRecord;
    })
    .filter((finding) => {
  ...
  await UPDATE(entities.DevelopmentObjects).set({ latestScoringImportId: "" }).where({ systemId: scoringImport.systemId });

  const developmentObjectMap = await getDevelopmentObjectMap();
  let progressCount = 0;
  const chunkSize = 1000;
  for (let i = 0; i < scoringRecordList.length; i += chunkSize) {
    LOG.info(`Processing ${i} to ${i + chunkSize} (${scoringRecordList.length}`);
    ...
    for (const scoringRecord of chunk) {
      ...
      if (!developmentObjectDB) {
        ...
        if (!developmentObject.devClass || !developmentObject.objectName || !developmentObject.objectType || !developmentObject.systemId) {
        ...
        if (developmentObjectDB.latestScoringImportId !== scoringImport.ID) {
          ...
          // Update the score
          if (!developmentObjectDB.devClass || !developmentObjectDB.objectName || !developmentObjectDB.objectType || !developmentObjectDB.systemId) {
          ...
          developmentObjectDB.namespace = determineNamespace(developmentObjectDB);
          developmentObjectDB.cleanCoreScore = await calculateCleanCoreScore(developmentObjectDB);
          developmentObjectDB.cleanCoreLevel_code = await determineCleanCoreLevel(developmentObjectDB);

          developmentObjectUpsert.push(developmentObjectDB);
          upsertCount++;
        } else {
          LOG.debug("Development Object already scored", { developmentObjectDB });
        }
      }
    }
    if (developmentObjectUpsert.length > 0) {
      await UPSERT.into(entities.DevelopmentObjects).entries(developmentObjectUpsert);
      if (tx) {
        await tx.commit();
      }
    }
...
export const importScoringById = async (scoringImportId, tx: Transaction, updateProgress?: (progress: number) => Promise<void>) => {
...
export const importLanguageVersion = async (languageVersionImport: Import, tx?: Transaction, updateProgress?: (progress: number) => Promise<void>) => {
  ...
  const languageVersionRecordList = result.data
    .map((finding) => {
      return {
        ...
        objectName: finding.objectName || finding.OBJECTNAME,
        devClass: finding.devClass || finding.DEVCLASS,
        languageVersion_code: finding.languageVersion || finding.LANGUAGEVERSION || finding.LanguageVersion || finding.languageversion,
        creationDate: finding.creationDate || finding.CREATIONDATE || finding.creationdate || finding.CREATION_DATE
      };
    })
    .filter((finding) => {
      if (!finding.objectType || !finding.objectName) {
  ...
  if (languageVersionRecordList == null || languageVersionRecordList.length == 0) {
  ...
  for (let i = 0; i < languageVersionRecordList.length; i += chunkSize) {
    ...
    for (const languageVersion of chunk) {
      ...
      if (!developmentObjectDB) {
        ...
        upsertCount++;
      } else if (developmentObjectDB.languageVersion_code !== languageVersion.languageVersion_code) {
        LOG.error("Language Version changed for scored object", { developmentObjectDB });
        developmentObjectDB.languageVersion_code = languageVersion.languageVersion_code;
        developmentObjectDB.cleanCoreLevel_code = await determineCleanCoreLevel(developmentObjectDB);
        developmentObjectUpsert.push(developmentObjectDB);
        upsertCount++;
      }
...
export const importLanguageVersionById = async (languageVersionImportId, tx?: Transaction, updateProgress?: (progress: number) => Promise<void>) => {
...
export const determineCleanCoreLevelAll = async (tx: Transaction, updateProgress: (progress: number) => Promise<void>) => {
  const developmentObjects = await SELECT.from(entities.DevelopmentObjects).columns(getDevelopmentObjectColumnsForCleanCoreLevel);

  let progressCount = 0;
  const chunkSize = 50;
  for (let i = 0; i < developmentObjects.length; i += chunkSize) {
    LOG.info(`Processing ${i} to ${i + chunkSize} (${developmentObjects.length})`);
    ...
    for (const developmentObject of chunk) {
      ...
      if (developmentObject.cleanCoreLevel_code !== cleanCoreLevel) {
        updatePromises.push(UPDATE.entity(entities.DevelopmentObjects).set({ cleanCoreLevel_code: cleanCoreLevel }).where({
          objectType: developmentObject.objectType,
          objectName: developmentObject.objectName,
          devClass: developmentObject.devClass,
          systemId: developmentObject.systemId,
