...
define view entity ZKNSF_I_ATC_UNCLASSIFIED
  as select from    ZKNSF_I_ATC_FINDINGS     as fnd
    ...
    left outer join ALL_CDS_STOB_VIEWS       as cds             on cds.CDSName = fnd.refObjectName
{
  ...
  key     cast( fnd.refApplicationComponent as zknsf_app_comp )                                               as applicationComponent,
          cast( case fnd.refSubType when 'STOB' then 'CDS_STOB'   when 'DDLS' then 'CDS_STOB'  else fnd.refSubType end   as zknsf_sub_type) as subType,
